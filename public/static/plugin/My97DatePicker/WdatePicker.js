...
;(function() {
  ...
  function B() {
    ...
    obj = {
          ...
          if ($) {
            for (const B in $) {
              if (this.dt[B] === undefined) {
                ...
                if (B == 'M') {
                  const C = $.M > 0 ? 1 : 0
                  const A = new Date(this.dt.y, this.dt.M, 0).getDate()
      ...
      show: function() {
        let A = U[M].getElementsByTagName('div'),
          $ = 100000
        for (let B = 0; B < A.length; B++) {
          const _ = parseInt(A[B].style.zIndex)
          if (_ > $) $ = _
        }
  ...
  function K(A, $, B) {
    const D = X[M][C]('HEAD').item(0)
    const _ = X[M].createElement('link')
    if (D) {
      _.href = A
      _.rel = 'stylesheet'
      _.type = 'text/css'
  ...
  function D($) {
    const _ = $ ? $.srcElement || $.target : null
    try {
      if (
        $dp.cal &&
        !$dp.eCont &&
  ...
  function I(G, _) {
    const D = G.el ? G.el.nodeName : 'INPUT'
    ...
    if (!G.eCont) for (const C in G) $dp[C] = G[C]
    if (
      !$dp.dd ||
      G.eCont ||
    ...
    function E(F, E) {
      F.innerHTML =
        '<iframe hideFocus=true width=97 height=9 frameborder=0 border=0 scrolling=no></iframe>'
      const D = F.lastChild.contentWindow[M]
      const _ = $.$langList
      const C = $.$skinList
      const H = E.getRealLang()
      F.lang = H.name
      F.skin = E.skin
      const G = [
        '<head><script>',
      ...
      G.push(
        '<script>var t;t=t||setInterval(function(){if(document.ready){new My97DP();$cfg.onload();$c.autoSize();$cfg.setPos($dp);clearInterval(t);}},20);if($FF||$OPERA)document.close();</script>'
      )
      E.setPos = B
      E.onload = Y
      D.write('<html>')
    ...
    function B(I) {
      ...
      const D = L(U)
      const A = Z(U)
      const E = $dp.dd.offsetHeight
      const _ = $dp.dd.offsetWidth
      if (isNaN(B)) B = 0
