...
function SelectBox(b, c, d, e) {
  ...
  var c = (this.data = c || [])
  var e = (this.opts = e || {})
  const f = (this.dataName = e.dataName || 'name')
  const g = (this.dataId = e.dataId || 'id')
  const h = (this.fontSize = e.fontSize || '14')
  const i = e.optionFontSize || '14'
  const j = (this.textIndent = e.textIndent || 4)
  const k = (this.color = e.color || '#000')
  const l = e.optionColor || '#000'
  const m = e.arrowColor || '#5c5c5c'
  const n = e.borderColor || '#c2c2c2'
  const o = (this.hoverColor = e.hoverColor || '#6fc')
  const p = e.backgroundColor || '#fff'
  const q = (this.placeholder = e.placeholder || '')
  const r = (this.defalut = e.defalut || '')
  const s = (this.width = e.width || 130)
  const t = (this.height = e.height || 26)
  const u = e.optionMaxHeight || 350
  const v = (this.borderWidth = e.borderWidth || 1)
  const w = e.borderRadius || 0
  const x = typeof e.arrowBorderWidth === 'undefined' ? v : e.arrowBorderWidth
  const y = (this.allowInput =
  ...
  const A = (this.ul = b.find('ul'))
  const B = (this.input = b.find('input'))
  const C = (this.retur = e.retur || false)
  this.changeList(c, {
    retur: C
  })
  const D = b.css('position') == 'static' ? 'relative' : b.css('position')
  b.css({
    width: s,
    height: t,
    position: D,
  ...
  const F = parseInt(t / 4)
  const G = parseInt((t * 3) / 8)
  b.find('i em').css({
    display: 'block',
    width: '0px',
...
SelectBox.prototype.addEvent = function() {
  ...
  this.allowInput &&
    l.focus(function() {
      ...
      f.on('blur', h)
      const j = function(a) {
        const b = k.children('li:visible').length
        switch (a.keyCode) {
          case 40:
            o.n++
            if (o.n > b) {
