...
export default {
  ...
  mounted() {
    ...
    let color = ''
    if (this.type === 'font') {
      color = this.holder.fgcolour
    }
    if (this.type === 'b') {
      color = this.holder.olcolour
      $(this.$refs.color)
        .parent()
        .css('padding', '4px')
    }
    if (this.type === 'backg') {
      color = this.holder.bgcolour
    }
    $(this.$refs.color).spectrum({
      preferredFormat: true,
      color: color /* that.activechunk.chunk.filter[that.index].fgcolour */,
      allowEmpty: true,
      showInitial: true,
      // showAlpha: true,
      showPalette: true,
